document.addEventListener('DOMContentLoaded', function () {
    ...
    const analogyForm = document.getElementById('analogyForm');
    if (!analogyForm) {
        return;
    }

    analogyForm.addEventListener('submit', generateAnalogies);
    ...
    async function generateAnalogies(event) {
        ...
        if (!errorDiv || !resultDiv || !loadingDiv || !conceptInput) {
            return;
        }

        if (!window.apiManager?.getApiKey()) {
            utils.showError(errorDiv, 'Please set up your API key using the settings menu (⚙️).');
            return;
        }
        ...
        if (!concept) {
            utils.showError(errorDiv, 'Please enter a concept to explain.');
            return;
        }
